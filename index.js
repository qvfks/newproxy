// index.js — Netlify Function Roblox Proxy
...
// Netlify serverless entrypoint
export async function handler(event) {
    const path = event.rawUrl ? new URL(event.rawUrl).pathname.split("/") : event.path.split("/");
    ...
    const targetURL = `https://${path[1]}.roblox.com/${path.slice(2).join("/")}${event.rawQuery ? `?${event.rawQuery}` : ""}`;
